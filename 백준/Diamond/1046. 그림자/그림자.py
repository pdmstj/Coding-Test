def count_unreachable_area(h, w, box):
    count = result = 0
    p, slopeList = [], []
    ...
    for i in range(len(p)):
        j = (i + 1) % len(p)
        result += abs((Light[0] * p[i][1] + p[i][0] * p[j][1] + p[j][0] * Light[1]) - (
                p[i][0] * Light[1] + p[j][0] * p[i][1] + Light[0] * p[j][1])) / 2

    return f'{w * h - result - count:.13f}'

h, w = map(int, input().split())
box = [input() for _ in range(h)]

print(count_unreachable_area(h, w, box))